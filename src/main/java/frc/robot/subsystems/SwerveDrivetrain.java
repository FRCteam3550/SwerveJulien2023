...
import frc.robot.subsystems.swerve.GamepadInputs;
...
public class SwerveDrivetrain extends SubsystemBase implements Chassis {
    ...
    private static final double MAX_VELOCITY_MS = // = 3.225 m/s
            5000.0 / 60.0 * // = 83.33 rot/s
                    SdsModuleConfigurations.MK4_L1.getDriveReduction() * // (14.0 / 50.0) * (25.0 / 19.0) * (15.0 /
                                                                         // 45.0) = 0.1228
    ...
    private final Odometry m_odometry = new Odometry(getGyroscopeRotation(), getModulePositions(), m_kinematics,
            m_fieldTracker);
    ...
    private GamepadInputs getJoystickInputs() {
    ...
    private void applyGamepadInputs(GamepadInputs inputs) {
    ...
    public Command goToFrontOfTag0() {
        return autoCommand(m_pathFollowing.goTo(DEVANT_TAG_0));
    }

    public Command zigzag() {
        return autoCommand(
            m_pathFollowing.robotRelativeMove(
                List.of(
    ...
    public Command autoCommand(Command followCommand) {
        return runOnce(m_odometry::deactivateCameraOdometry)
            .andThen(followCommand)
            .andThen(runOnce(m_odometry::activateCameraOdometry));
    }
    ...
    public Command play() {
        return runOnce(m_replay::startPlaying)
                .andThen(run(() -> {
                    var inputs = m_replay.nextInputsToPlay();
                    if (inputs.isPresent()) {
                        applyGamepadInputs(inputs.get());
                    }
                }))
    ...
    private void updateActualSpeeds() {
        ...
        for (int i = 0; i < m_modules.length; i++) {
            var module = m_modules[i];
            actualStates[i] = new SwerveModuleState(module.getDriveVelocity(),
                    Rotation2d.fromRadians(module.getSteerAngle()));
        }
